...
export const getBranchDetailsController = asyncHandler(async (req, res) => {
  const { id } = req.params;
...
export const createBranchController = asyncHandler(async (req, res) => {
  const { name, address, availableRadiology } = req.body;
...
export const updateBranchController = asyncHandler(async (req, res, next) => {
  ...
  const updatedBranch = await Branches.findByIdAndUpdate(id, update, {
    new: true,
    runValidators: true,
  }).populate("availableRadiology.radiology");
  if (!updatedBranch) {
    return res.status(400).json(new ApiResponse(404, null, "Branch not found"));
  }
